...
const empty={name:'',designation:'',company:'',email:'',phone:'',address:'',taxId:'',tags:'',notes:''};
...
export default function ClientForm(){
  ...
  const fill=(d:any)=>{setF(p=>({...p,
    ...
    tags:Array.isArray(d.tags)&&d.tags.length?d.tags.join(', '):p.tags,
...
'+p.notes:'')):p.notes}));setMsg('✅ Card read hoyeche. Check kore Save koro.')};
  ...
  async function save(){
    ...
    setBusy(true);
    const r2=await fetch('/api/clients',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(f)});
    setBusy(false);
    if(r2.ok){setF({...empty});setMsg('✅ Client saved');r.refresh()}
  ...
  return <div className="card">
    ...
    <div className="form">
      ...
      <input placeholder="Designation (e.g. Marketing Manager)" value={f.designation} onChange={e=>set('designation',e.target.value)}/>
      ...
      <div className="row"><input placeholder="TIN / Tax ID" value={f.taxId} onChange={e=>set('taxId',e.target.value)}/><input placeholder="Tags (comma separated)" value={f.tags} onChange={e=>set('tags',e.target.value)}/></div>
      <textarea placeholder="Notes" value={f.notes} onChange={e=>set('notes',e.target.value)}/>
      {msg&&<div className="muted">{msg}</div>}
      <button className="btn" onClick={save} disabled={busy}>{busy?'Saving…':'Save client'}</button>
    </div>
  </div>
}
